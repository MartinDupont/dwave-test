...
class EliminationStrategy:
    ...
    def solve(self, **kwargs):
        ...
        if isinstance(self.sampler, neal.SimulatedAnnealingSampler):
        ...
        solution_set = {} # keys with True will indicate that it is a valid solution.
        for x_vec, y in zip(x_data, y_data):
            poly = c.make_polynomial_for_datapoint(y, x_vec)
            bqm = c.make_bqm(poly, offset)
            result = sampler.sample(bqm, num_reads=5)
            for datum in result.data(['sample', 'energy', 'num_occurrences']):
                solution = self.convert_dict_to_tuples(datum.sample)
                if solution_set.get(solution, True):
                    solution_set[solution] = self.check_solution(x_vec, y, datum.sample)
                    # Value can only ever change from true to false

        valid_solutions = [sol for sol, val in solution_set.items() if val]
